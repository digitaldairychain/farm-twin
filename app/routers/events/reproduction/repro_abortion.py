...
async def create_repro_abortion_event(request: Request, repro_abortion: ReproAbortion):
    ...
    """
    model = repro_abortion.model_dump(by_alias=True, exclude=["ft"])
    return await add_one_to_db(model, request.app.state.repro_abortion, ERROR_MSG_OBJECT)
