...
def get_measurements(device, sensor, start, end):
    ...
    q_string = ("""SELECT value FROM "measurements" WHERE node =~ /^""" +
                device + """$/ AND sensor =~ /^""" + sensor
                + """$/ AND time >= '""" + start.isoformat()
                + """Z' AND time <= '""" + end.isoformat() + """Z'""")
...
def get_measurements_async(start, end):
    ...
    for device in devices.items():
        with concurrent.futures.ThreadPoolExecutor(max_workers=MW) as executor:
            _ = {executor.submit(get_measurements, device[0], sensor,
                                 start, end): sensor for sensor in device[1]}
...
def get_sensors(device):
    ...
    """
    data = query_db('SHOW TAG VALUES WITH KEY = sensor WHERE node =~ /^' +
                    device + '$/ AND channel !~ /^link-/')
...
def get_sensors_async():
    ...
    """
    with concurrent.futures.ThreadPoolExecutor(max_workers=MW) as executor:
        _ = {executor.submit(get_sensors, device):
             device for device in devices}


def query_db(query):
    """Send a query to the remote Decentlab API."""
    request = requests.get(config['url'], params={'db': 'main', 'epoch': 'ms',
                                                  'q': query},
...
def load_config():
    ...
    _config['end_date'] = datetime.datetime.strptime(_config['end_date'],
                                                     '%d/%m/%Y')
    try:
